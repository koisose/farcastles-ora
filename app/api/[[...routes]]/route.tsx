...
//   const saved = await saveData({  vectorContent: embed.values, text: data.text }, "microsoft-learn");
...
//  const firstHalf = data.text.substring(0, Math.floor(data.text.length / 2));
//  const secondHalf = data.text.substring(Math.floor(data.text.length / 2));
//  const embed1=await makeEmbedding(firstHalf)
//  const embed2=await makeEmbedding(secondHalf)
//  const saved=await saveData({vectorContent:embed1.values,url:data.url},"microsoft-learn")
//  const saved2=await saveData({vectorContent:embed2.values,url:data.url},"microsoft-learn")
